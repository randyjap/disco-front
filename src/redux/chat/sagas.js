...
const {
  database, createBatch, rsfFirestore, createNewRef,
} = FirebaseHelper;
const fsProps = {};
...
const readUsers = async () => await fsProps.usersCollections.get().then((querySnapshot) => {
  const users = [];
  try {
...
const readChatrooms = async () => await fsProps.chatroomsUserCollections.get().then((querySnapshot) => {
  const chatRooms = [];
  try {
    querySnapshot.forEach((doc) => {
...
const readMessages = async ({ id }) => await fsProps.messagesCollections
  .where('chatRoomId', '==', id)
  .get()
  .then((querySnapshot) => {
    const messages = [];
...
function* initChat() {
  ...
  const messages = chatRooms.length === 0 ? [] : yield call(readMessages, chatRooms[0]);
